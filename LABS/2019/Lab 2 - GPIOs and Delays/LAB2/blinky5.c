...
void delay_100ms_blocking(void) 
{
	...
	for(i=0; i<SystemCoreClock/18; i++){__NOP();}		//incrementing a variable from 0 to 4,500,000 takes 100ms when MCU speed is 180MHz
}
...
void LED_TOGGLE (unsigned int BIT) 
{
			if ((GPIOB->ODR)& 1<<BIT) GPIOB->BSRR = 1 << (16+BIT) ;
			else GPIOB->BSRR = 1 << (0+BIT) ;
}
